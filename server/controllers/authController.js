...
exports.register = async (req, res) => {
    ...
    try {
        const [rows] = await db.query('SELECT * FROM otps WHERE email = ? AND otp = ? AND expires_at > NOW()', [email, otp]);
        ...
        if (userExists.length > 0) return res.status(400).json({ message: 'User with this email already exists.' });
...
exports.login = async (req, res) => {
    ...
    try {
        // CORRECTED: Fetch ALL columns for the user with '*' to ensure all data is available.
        ...
        // CORRECTED: Send back the COMPLETE user object to be stored in localStorage.
        res.json({
            token,
            user: { 
                id: user.id, 
